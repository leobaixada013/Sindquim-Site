...
O Playwright valida o portal em três perfis: Android (Pixel 5), iPhone 13 com WebKit e uma tela compacta de 320 px.
...
Com o portal Docker disponível em `http://localhost:4421`, execute:

```bash
npm run test:e2e:mobile
```

Para testar outro ambiente, informe a URL sem iniciar um servidor local:

```bash
PLAYWRIGHT_BASE_URL=https://portal.exemplo.org npm run test:e2e:mobile
```

O relatório HTML fica em `playwright-report/` e pode ser aberto com `npm run test:e2e:report`.
